...
public class Delete extends BaseCommand {
	...
	public CommandResult execute(CommandSource src, CommandContext args) throws CommandException {
		...
		String type = (String) args.getOne("block/npc").get();
		if (type.equalsIgnoreCase("block")) {
			...
			if (hitOpt.isPresent()) {
				List<Trigger> trigger = storageManager.getTriggerAt(hitOpt.get().getLocation());
				if (trigger == null) {
					throw new CommandException(Text.of("Invalid trigger!"));
				}
				for (Trigger t : trigger) {
					storageManager.removeTrigger(t);
				}
				src.sendMessage(Text.of(TextColors.GREEN, "Successfully removed trigger!"));
			} else {
			...
			src.sendMessage(
					Text.of(TextColors.GRAY, "Right click the entity the entity that you want to remove the trigger!"));

			Sponge.getEventManager().registerListeners(PixelBuiltQuests.instance, new OneTimeHandler((e, player) -> {
				if (player.getUniqueId().equals(p.getUniqueId())) {
					List<Trigger> trigger = storageManager.getTrigger(e.getTargetEntity());
					if (trigger == null || trigger.isEmpty()) {
						player.sendMessage(Text.of(TextColors.GREEN, "There's no trigger associated with this NPC!"));
					} else {
						storageManager.removeNPC(e.getTargetEntity());
						e.getTargetEntity().remove();
						player.sendMessage(Text.of(TextColors.GREEN, "Successfully removed quest from this entity!"));
					}
				}
			}));
		} else {
